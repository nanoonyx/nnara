...
VER = "nmaster_0215"  # comment out wdt from nmaster_020e.py
...
def msend(msg, target_id="a"):
    ...
    try:
        ...
        if target_id_str == "b":  # broadcast
            esp.send(bcast, msg.encode())
            return True
        elif target_id_str == "a":  # send to all in sids
            try:
                esp.send(msg.encode())
            except:
                return True
            return True
        elif target_id_str == "0":  # send to sids, one by one with error check
            for s in sids:
                try:
...
def handle_mdebug(parts):
    ...
    if len(parts) > val_idx:
        try:
            debug_mode = int(parts[val_idx])
        except ValueError:
            pass
    print_response("A", "MDEBUG", debug_mode)
...
def scmd_forward(cmd, parts, sid, line):
    ...
    if cmd in MELK:
        ...
        # else:
        #     msg_to_send = cmd + val
        if msend(msg_to_send, sid):
            log(msg_to_send)
        else:
            print_response("NG", "msend", line)
    else:
        log(f"{cmd},unknown command")
...
def init_system():
    ...
    global mid, ch, debug_mode, config_data, sids, sid_data, mqtt_client, esp, poller, sm
    ...
    try:
        ...
        debug_mode = config_data.get("debug", 0)

        # Load SID mapping
        msids_file = f"msids_{mid.lower()}.json"
        # sid_data = read_json_file(msids_file)
        # sids = sid_data.get("sids", {})
        # use msids instead of sids
        msids = read_json_file(msids_file)
    except Exception as err:
    ...
    ## Add SID Peers
    for m in msids.values():
        try:
            esp.add_peer(binascii.unhexlify(m))
        except:
            pass
