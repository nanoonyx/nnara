...
# Constants
VER = "nslave_0214"  # modified from nslave_0210e. wdt
CONFIG_FILE = "nslave.json"
pmacs_file = "pmacs.json"
...
class SlaveNode:
    def __init__(self):
        ...
        pids_file = f"pids_{self.sid}.json"

        try:
            self.pids = read_json_file(pids_file, {})
            self.pmacs = list(self.pids.values())
            self.p4dict = {item[-4:]: item for item in self.pmacs}
            self.lmacs = list(self.pmacs)
        except Exception as err:
            self.sos = f"{pids_file}, {err}"
    ...
    async def cmd_pmacs(self, code, target_pmacs, timeout=3500, repeat=1):
        start_tick = time.ticks_ms()
        failed_macs = []
        # wdt.feed()
        for p in target_pmacs:
            ...
            try:
                ...
                self.send_msg(f"cmd_pmacs Exception {p[-4:]}: {err}")
        ...
        self.log(
            f"N={len(target_pmacs)},F={len(failed_macs)},T={time.ticks_diff(time.ticks_ms(), start_tick)}"
        )
        return failed_macs
    ...
    async def handle_hex_cmd(self, cmd, parts, msg, last_tick):
        ...
        if len(parts) > 1 and parts[1]:  # valid target list provided
            ...
            failed = await self.cmd_pmacs(code, target_list, timeout=5000)
            if failed:
                # Retry once for specific targets
                failed = await self.cmd_pmacs(code, failed, timeout=5000)
                self.send_msg(
                    f"NG,{[f[-4:] for f in failed]},{time.ticks_diff(time.ticks_ms(), last_tick)}"
                )
            else:
                self.send_msg(
                    f"OK,{p4_list},{time.ticks_diff(time.ticks_ms(), last_tick)}"
                )

        else:  # Broadcast to all pmacs
            failed = await self.cmd_pmacs(code, self.pmacs, timeout=3000)
            retry_count = 0
            while failed and retry_count < 2:
                retry_count += 1
                # self.send_msg(f"NG,{[d[-4:] for d in failed]},Retry:{retry_count}")
                failed = await self.cmd_pmacs(code, failed, timeout=3500)

            if failed:
                self.send_msg(f"NG,{[d[-4:] for d in failed]}")
            else:
                self.send_msg(
                    f"{cmd},OK,D:{time.ticks_diff(time.ticks_ms(), last_tick)}"
                )

    async def handle_scan_cmd(self, cmd, parts, last_tick):
        if cmd == "SCAN":
            target_list = list(self.pmacs)
            failed = await self.cmd_pmacs(CODE_RGB, target_list, timeout=3000)
            status = "NG" if failed else "OK"
            failed_str = str([l[-4:] for l in failed]) if failed else ""
            self.send_msg(
                f"{cmd},{status},{failed_str},D:{time.ticks_diff(time.ticks_ms(), last_tick)}"
            )
            ...
            if cmd == "ZSCAN":
                found_rssi = [
                    d.rssi for d in devices
                ]  # just list rssi separately or combined?
                # Original logic sorted by RSSI
                combined = sorted(
                    zip(found_macs, found_rssi), key=lambda x: x[1], reverse=True
                )
                ...
                write_json_file(pmacs_file, found_macs)
                await asyncio.sleep(0.1)
                self.pmacs = read_json_file(pmacs_file, {})
                self.lmacs = list(self.pmacs)
                self.p4dict = {item[-4:]: item for item in self.pmacs}  # Update dict
                self.send_msg(
                    f"{cmd},{[p[-4:] for p in self.pmacs]},D:{time.ticks_diff(time.ticks_ms(), last_tick)}"
                )
    ...
    async def handle_mac_file_cmd(self, cmd, parts):
        # PMACS, ZMACS, LMACS management
        if cmd in ["PMAC", "PMACS"]:
            self.send_msg(f"{cmd},{[p[-4:] for p in self.pmacs]}")
        ...
        elif cmd in ["PID", "PIDS"]:
            self.send_msg(f"{cmd},{self.pids}")
            ...
            # Determine source
            data = self.pmacs
            if "PMACS" in cmd or "CSAVE" in cmd:
                data = self.pmacs
            elif "LMAC" in cmd or "LSAVE" in cmd:
                data = self.lmacs
            elif "ZMAC" in cmd or "ZSAVE" in cmd:
                data = self.zmacs

            write_json_file(pmacs_file, data)
            if self.debug_mode:
    ...
    async def handle_system_cmd(self, cmd, parts, last_tick):
        if cmd in ["VERSION", "VER", "V"]:
            ...
            if self.debug_mode:
                self.send_msg(
                    f"{cmd},S:{self.sid},D:{time.ticks_diff(time.ticks_ms(), last_tick)}"
                )
    ...
    async def process_msg(self, peer, msg, last_tick):
        ...
        if cmd == "FWUPDATE":
            ...
            x in cmd
            for x in ["PMAC", "ZMAC", "LMAC", "PID", "PIDS", "CSAVE", "LSAVE", "ZSAVE"]
        ):
    ...
    async def run(self):
        ...
        while True:
            ...
            if time.ticks_diff(time.ticks_ms(), last_tick) > 300000:
                # wdt.feed()
                sm = f"B:{adc.read_uv()/500000:.2f},T:{esp32.mcu_temperature()}"
                self.send_msg(f"BAT,{sm}")
                self.log(micropython.mem_info())
                last_tick = time.ticks_ms()
                gc.collect()
